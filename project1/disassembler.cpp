...
void disassembleInstr(uint32_t pc, uint32_t instr) {
  ...
  opcode = (instr >> 26) & 0x3F;  // 0xFC000000                              // 2nd Attempt
  rs =     (instr >> 21) & 0x1F;  // 0x03E00000
  rt =     (instr >> 16) & 0x1F;  // 0x001F0000
  rd =     (instr >> 11) & 0x1F;  // 0x0000F800
  shamt =  (instr >> 6)  & 0x1F;  // 0x000007C0       
  funct =  instr & 0x0000003F;    // 0x0000003F
  uimm =   (instr << 16) >> 16;
