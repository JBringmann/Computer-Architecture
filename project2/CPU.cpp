...
void CPU::decode() {
  ...
  switch(opcode) {
    case 0x00:
      switch(funct) {
        case 0x00: D(cout << "sll " << regNames[rd] << ", " << regNames[rs] << ", " << dec << shamt);
                   writeDest = true;             // R[rd] = R[rt] << shamt
                   aluOp = SHF_L;
                   destReg = rd;           
                   aluSrc1 = regFile[rs];
                   aluSrc2 = shamt;
        ...
        case 0x10: D(cout << "mfhi " << regNames[rd]);
                   writeDest = true;             // op(ALU_OP op, uint32_t src1, uint32_t src2);  
                   aluOp = ADD;                  // R[rd] = Hi
                   destReg = rd;
                   aluSrc1 = hi;
                   aluSrc2 = regFile[REG_ZERO]; 
        ...
        case 0x18: D(cout << "mult " << regNames[rs] << ", " << regNames[rt]);
                   writeDest = false;            // false because the result is stored in REG_HILO
        ...
        case 0x23: D(cout << "subu " << regNames[rd] << ", " << regNames[rs] << ", " << regNames[rt]);
                   ...
                   destReg = rd;
                   aluSrc1 = regFile[rs];
                   aluSrc2 = -(regFile[rt]);     // negative because we only have an aluOp ADD, no SUBTRACT
                   break;
        case 0x2a: D(cout << "slt " << regNames[rd] << ", " << regNames[rs] << ", " << regNames[rt]);
                   writeDest = true;             // R[rd] = (R[rs] < R[rt]) ? 1 : 0
                   aluOp = CMP_LT;
                   destReg = rd;
                   aluSrc1 = regFile[rs];
                   aluSrc2 = regFile[rt];
    ...
    case 0x04: D(cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2));
               if(regFile[rs] == regFile[rt])     // from green card
    ...
    case 0x23: D(cout << "lw " << regNames[rt] << ", " << dec << simm << "(" << regNames[rs] << ")");
               writeDest = true;               // R[rt] = M[R[rs] + SignExtImm]
               opIsLoad = true;
               aluOp = ADD;
               destReg = rt;
               aluSrc1 = regFile[rs];          // M[R[rs]]  
               aluSrc2 = simm;
