...
void CPU::decode() {
  ...
  switch(opcode) {
    ...
    case 0x04: {       
               ...
               uint32_t pcBranch = pc - 4;
               uint32_t predTarget = 0; 
               bool predTaken = branchPred.predict(pcBranch, predTarget); 
               bool actualTaken = (regFile[rs] == regFile[rt]) ? true : false;
            
               if(actualTaken)
                  pc = pc + (simm << 2); //caculate target address to jump to
               if(branchPred.isMispredict(predTaken, predTarget, actualTaken, pc))
                  stats.flush(2);                  
               branchPred.update(pcBranch, actualTaken, pc); // pc = target address
               break;                           
    }
    case 0x05: {       
