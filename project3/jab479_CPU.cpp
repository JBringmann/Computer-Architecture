...
void CPU::decode() {
  ...
  switch(opcode) {
    case 0x00:
      switch(funct) {
        ...
        case 0x10: D(cout << "mfhi " << regNames[rd]);
                   ...
                   aluOp = ADD;                  // R[rd] = Hi
                   aluSrc1 = hi; stats.registe(REG_HILO);
                   aluSrc2 = regFile[REG_ZERO]; 
                   break;
        case 0x12: D(cout << "mflo " << regNames[rd]);
                   writeDest = true; destReg = rd; stats.registerDest(rd);             // op(ALU_OP op, uint32_t src1, uint32_t src2); 
                   aluOp = ADD;                  // R[rd] = Lo
                   aluSrc1 = lo; stats.registe(REG_HILO);
                   aluSrc2 = regFile[REG_ZERO];
                   break;
    ...
    case 0x03: D(cout << "jal " << hex << ((pc & 0xf0000000) | addr << 2)); // P1: pc + 4
               writeDest = true; destReg = REG_RA; stats.registerDest(ra)                 // writes PC+4 to $ra
...
void CPU::printFinalStats() {
  ...
  cout << "Cycles: " << stats.getCycles() << endl;
       << "CPI: " << fixed << setprecision(2) << 1.0 * (stats.getCycles() / (double)instructions) << endl << endl;
